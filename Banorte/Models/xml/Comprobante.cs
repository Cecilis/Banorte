...
namespace Banorte.Models.xml
{
    ...
    public class Comprobante
    {
        ...
        public string getExpresionImpresa(string versionCFDI)
        {
            string expresionImpresa = string.Empty;

            if (versionCFDI == "3.2")
            {
                expresionImpresa = "?re=" + this.Emisor.rfc + "&rr=" + this.Receptor.rfc + "&tt=" + this.total + "&id=" + this.Complemento.TimbreFiscalDigital.UUID;
            }
            else
            {
                expresionImpresa = "?re=" + this.Emisor.Rfc + "&rr=" + this.Receptor.Rfc + "&tt=" + this.Total + "&id=" + this.Complemento.TimbreFiscalDigital.UUID;
